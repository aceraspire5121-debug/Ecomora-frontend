...
styleTag.textContent = `
  ...
  .ecm-register-root::before,
  .ecm-register-root::after {
    content: '';
    position: absolute;
    width: 260px;
    height: 260px;
  ...
  .ecm-register-root::before { top: -100px; left: -100px; }
  .ecm-register-root::after  { bottom: -120px; right: -120px; }

  .ecm-card {
    animation: ecmFadeUp .55s cubic-bezier(.22,.68,0,1.2) both;
  }
  @keyframes ecmFadeUp {
  ...
  .ecm-divider::before, .ecm-divider::after {
    content: '';
    flex: 1;
    height: 1px;
...
`;
if (!document.head.querySelector("[data-ecm-register]")) {
  styleTag.setAttribute("data-ecm-register", "1");
...
const RegisterPage = () => {
  ...
  return (
    <Box className="ecm-register-root">
      {/* ── wordmark ── */}
      <Box
        sx={{
          position: "absolute",
          ...
          <Box display="flex" flexDirection="column" gap={1.8}>
            ...
            <TextField
              ...
              {...register("password", {
                ...
                maxLength: {
                  value: 15,
                  message: "Password should be of maximum 15 characters",
                },
                pattern: {
                  value: /^(?=.*[A-Za-z])(?=.*\d).{6,}$/,
                  message:
            ...
            {/* divider */}
            <Box className="ecm-divider">already have an account?</Box>
